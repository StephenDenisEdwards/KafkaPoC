...
namespace KafkaConsumer_StrictOrder
{
	class Program
	{
		...
		{
			await StandardConsole.Start();
			//await RefactoredConsole.Start();
		}
	}
	...
	class RefactoredConsole
	{
		...
		public static async Task Start()
		{
			...
			//ITopicManager topicMgr = new TopicManager(Bootstrap);
			//IPatientProducer producer = new PatientProducer(Bootstrap, SchemaReg);
			//IPatientConsumer consumer = new PatientConsumer(Bootstrap, SchemaReg, GroupId, DltTopic);

			using var topicMgr = new TopicManager(Bootstrap);
			using var producer = new PatientProducer(Bootstrap, SchemaReg);
			using var consumer = new PatientConsumer(Bootstrap, SchemaReg, GroupId, DltTopic);

			// -- ensure topics --
			await topicMgr.EnsureTopicExistsAsync(Topic, partitions: 1, replicationFactor: 1);
			await topicMgr.EnsureTopicExistsAsync(DltTopic, partitions: 1, replicationFactor: 1);
